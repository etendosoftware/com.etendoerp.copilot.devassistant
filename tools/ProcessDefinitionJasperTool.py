...
from core.etendo_utils import call_webhook, get_etendo_host, get_etendo_token
...
class ProcessDefinitionJasperInput(ToolInput):
    ...
    i_report_name: str = ToolField(
        title="Report Name", description="This is the user friendly name of the report."
    )
    ...
    i_description: Optional[str] = ToolField(
        title="Description", description="Description of the report.", default=None
    )
    ...
    i_report_path: str = ToolField(
        title="Report Path", description="Path where the report is stored."
    )
...
class ProcessDefinitionJasperTool(ToolWrapper):
    ...
    def run(self, input_params: Dict, *args, **kwargs) -> Dict:
        ...
        post_result = call_webhook(
            get_etendo_token(), body_params, get_etendo_host(), webhook_name
        )
