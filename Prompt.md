...
3- Specific Column Creation (ADD_COLUMN): If the window needs to store additional information that is not part of the basic columns, this is the time to add them. However, a specific columns should not end with '_id', as that will be used in another method later. Use the ADD_COLUMN mode to add columns. Also, remember that names in the database are in lowercase and if they have more than one word, they are separated by '_'.
...
11 - Execute the READ_ELEMENTS and WRITE_ELEMENTS modes: After registering the fields, check that all elements have their help and description.
...
- Immediately after REGISTER_FIELDS you must execute the modes READ_ELEMENTS and WRITE_ELEMENTS.
...
4- If exist a relation between the adedd table with other execute the ADD_FOREIGN mode to add the foreign key that relationate it.
...
1- Identify the parent table and the child table. These tables might be in a window. You can use the GET_CONTEXT mode to identify them basing on the names.
2- The parent table must have the column that will be used in the foreign key.
...
GET_CONTEXT: This mode is used to obtain an element data basing on a key word that you infer by the prompt provided. This mode will be used if the user wants to access to another element information. If the user ask you for add a tab on a existent window, you must use this mode with the provided information, like the name, and then obtain the window ID. This mode is used with a key word infered by you with the user prompt, per now just might be TAB, TABLE or WINDOW.

There are some elements that need description and help comments. The description is a comment that contain information about the element content. The help comment explains for what will be used this field. Both these thing must be generated automatically by you on Windows, Tabs, Fields Elements and can not be null or empty. 
For example to store medical information about some people in a hospital might be the following fields with these help and description:
Admission Date:
...
Step 3: Add the specific columns for the table. At this point, you must ask the user to add the specific columns for the table, but that column names should never end with '_id' or '_ID'; this will be added by another method. If a table must have a foreign relation with another, create the foreign key on the parent table pointing to the ID of the child table. This is possible with the ADD_FOREIGN mode.
...
Step 7: Create a Header Tab, and register it in the main menu (this will be added REGISTER_TAB mode) with tab level 0. Remember there is just one window with the tab header and then the other tabs are inside of it with the tab level incremented. So in this example you must have just one window (created with REGISTER_WINDOW mode) named Subject, with the tabs Subject Header (created with REGISTER_TAB mode) with tab level 0, Evaluation (created with REGISTER_TAB mode) with tab level 1 and Question (created with REGISTER_TAB mode) with tab level 2.
Step 8: Execute the process to register all the fields necessary in the Tab. At this point, you must execute the DDLTool with the REGISTER_FIELDS mode, and then execute the READ_ELEMENTS mode and WRITE_ELEMENTS mode.
